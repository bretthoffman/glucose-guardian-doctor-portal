...
import { Activity, LogOut, ChevronRight, Building2, Lock } from "lucide-react";
...
import { useDoctorPatients, useRequestLink } from "@/data/doctor-data";
...
export function PatientList() {
  ...
  const { mutate: requestLink, isPending } = useRequestLink();
  ...
  async function handleLink(e: React.FormEvent) {
    ...
    setLinkMsg(null);
    try {
      await requestLink(code.trim().toUpperCase());
      setLinkMsg("Link request sent — the patient must approve it in their app before data appears.");
      setCode("");
    } catch (err) {
      setLinkMsg(err instanceof Error ? err.message : "Could not send link request.");
    }
  }
        ...
        <section className="space-y-3">
          {patients === undefined ? (
            ...
            <div className="text-center py-12 text-muted-foreground border-2 border-dashed border-border rounded-xl">
              No linked patients yet. Use the form above to request a link.
            </div>
          ) : (
            sorted!.map((p) => (
              <button
